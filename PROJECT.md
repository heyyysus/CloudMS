...
**Not yet built** — automated email ships today (see **Correspondence and reminders** above), but automated SMS and AI-assisted features described above don't exist in code yet, and third-party/carrier integration so far is limited to VIN decoding and TurboRater rater-file import. The Home dashboard's client/policy/activity summary cards are still placeholders — the only real content there today is the rater-file drop target — and personal auto remains the only line of business modeled. Multi-tenancy's remaining pieces (see `docs/multitenancy.md`) are organization creation and first-admin bootstrap (inviting a user into an org that already exists ships today as `POST /users/invite`; what's missing is a route to create the organization and seat its first admin), organization settings columns to retire the last agency-level environment variables — `MAIL_REPLY_TO`, `REMINDER_TIMEZONE` and `REMINDER_SEND_HOUR` — together with per-organization scoping of the reminder planner, and a demo org living in the same deployment. The domain model's `clientPhones`/`clientEmails` tables already capture the contact data those future features will need.
...
2. **Mostly done (#117, #119, #120, #121, #122, #130).** Make the app multitenant so it can be released to more than one agency. Shipped: an `organizations` table, `org_id` on every tenant-owned row, organization-scoped repositories, row-level security as the backstop, per-organization invoice and receipt numbering, and opaque row ids. Remaining: organization settings columns for the last agency-level environment variables (`MAIL_REPLY_TO`, `REMINDER_TIMEZONE`, `REMINDER_SEND_HOUR`) plus per-organization planner scoping, an organization creation and first-admin bootstrap flow, and a demo org that lives in the same deployment. The full plan is `docs/multitenancy.md`.
