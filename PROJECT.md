...
- `Admin`, `ManageUsers`, `ManageCarriers`, `TrustAccounting`, `ReminderRules`, `CorrespondenceTemplates` — admin-facing screens for org users/carriers, trust-ledger review, and reminder/correspondence-template management
...
Automated email ships today: `backend/src/mailer.ts` sends through Resend, and `backend/src/jobs/{planner,dispatcher,scheduler}.ts` plan and dispatch reminder-rule sends with leader election and `FOR UPDATE SKIP LOCKED`, so running several app containers needs no extra configuration. Staff can also send one-off correspondence from an admin-authored template. Automated SMS and AI-assisted communication remain future work.
...
**Not yet built** — automated email ships today (see **Domain model** above), but automated SMS and AI-assisted features described above don't exist in code yet, and third-party/carrier integration so far is limited to VIN decoding and TurboRater rater-file import. The Home dashboard's client/policy/activity summary cards are still placeholders — the only real content there today is the rater-file drop target — and personal auto remains the only line of business modeled. Multi-tenancy's remaining pieces (see `docs/multitenancy.md`) are an organization creation/invite flow (today an org's first admin can't self-serve; there is no route to create an organization) and a demo org living in the same deployment. The domain model's `clientPhones`/`clientEmails` tables already capture the contact data those future features will need.
...
2. **Done (#117, #119, #122, #130).** Make the app multitenant so it can be released to more than one agency: an `organizations` table, `org_id` on every tenant-owned row, organization-scoped repositories, per-organization invoice and receipt numbering, organization settings in place of agency-level environment variables, an organization creation and invite flow, and a demo org that lives in the same deployment. Remaining: the organization creation/invite flow and the demo org. The full plan is `docs/multitenancy.md`.
...
5. Add automated SMS communication for renewals, document requests, and policy updates — email is done, see **Domain model** above.
