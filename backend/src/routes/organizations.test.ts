...
import { findMembership, findOrganizationBySlug, softDeleteUser } from "../repositories"
...
describe("POST /organizations", () => {
  ...
  it("returns 400 for a bad body", async () => {
    const owner = await ctx.platformOwner("org-create-badbody")
  ...
  it("creates the organization and seats the given email as its admin", async () => {
    const owner = await ctx.platformOwner("org-create-create")
    const cookie = await ctx.unboundCookie(owner.id)
    ...
    // Mail is not configured in tests, so this is the failure path - the
    // point of the assertion is that the send's outcome is reported at all,
    // the way POST /users/invite reports it.
  ...
  it("returns 409 for a duplicate slug and leaves no zero-admin organization behind", async () => {
    const owner = await ctx.platformOwner("org-create-dupe-slug")
    const cookie = await ctx.unboundCookie(owner.id)
  ...
  it("rolls back the organization when the admin email belongs to a deleted user", async () => {
    const owner = await ctx.platformOwner("org-create-rollback")
    const cookie = await ctx.unboundCookie(owner.id)
    // Just a deleted account, not an owner - what the route keys off of is
    // users.deleted_at, which inviteToOrg checks before anything else.
    const deletedUser = await makeTestUser("org-create-rollback-deleted")
    ctx.track("user", deletedUser.id)
